...
const focusAreaMapping = {
  ...
  'human rights': ['Human Rights', 'Civil Liberties', 'Justice', 'Rule of Law', 'Anti-Discrimination'],
  'media': ['Media Freedom', 'Journalism', 'Press Freedom', 'Digital Media', 'Media Literacy'],
  'women': ['Gender Equality', 'Women\'s Rights', 'Women\'s Empowerment', 'Gender-Based Violence Prevention'],
  ...
  'environment': ['Environmental Protection', 'Climate Change', 'Sustainability', 'Green Development'],
...
const eligibilityTemplates = {
  ...
  'default': 'Registered civil society organizations with proven track record and financial management capacity'
};
  ...
  try {
    ...
    for (const grant of grants) {
      ...
      if (!grant.focus_areas || grant.focus_areas.length < 10) {
        ...
        if (focusAreas.length === 0) {
          if (orgNameLower.includes('democracy')) {
            ...
            focusAreas.push('Civil Society Development', 'Community Development', 'Capacity Building');
          } else {
            focusAreas.push('Civil Society Development', 'Capacity Building', 'Community Development');
          }
        }
      ...
      if (!grant.eligibility_criteria || grant.eligibility_criteria.length < 20) {
        ...
        } else if (grantNameLower.includes('humanitarian') || grantNameLower.includes('emergency')) {
      ...
      if (!grant.duration || grant.duration === 'N/A') {
        if (grant.grant_name.toLowerCase().includes('emergency') || 
            grant.grant_name.toLowerCase().includes('urgent')) {
      ...
      if (!grant.program_type) {
        ...
        } else if (grant.grant_name.toLowerCase().includes('capacity') || 
                   grant.grant_name.toLowerCase().includes('institutional')) {
          updates.program_type = 'Institutional Support';
        } else if (grant.grant_name.toLowerCase().includes('fellowship') || 
    ...
    const { data: verifyGrants } = await supabase
      .from('grants')
      .select('id, grant_amount, country_region, focus_areas, eligibility_criteria, application_deadline, duration, program_type')
      .order('id');
    ...
    verifyGrants.forEach(grant => {
      ...
      if (!grant.eligibility_criteria || grant.eligibility_criteria.length < 20) stillMissing.eligibility++;
      if (!grant.application_deadline || grant.application_deadline === 'N/A') stillMissing.deadline++;
      if (!grant.duration || grant.duration === 'N/A') stillMissing.duration++;
      if (!grant.program_type) stillMissing.programType++;
    });
    
    console.log('\n📈 FINAL STATUS:');
    console.log(`Grants with amounts: ${verifyGrants.length - stillMissing.amounts}/${verifyGrants.length}`);
    console.log(`Grants with geographic info: ${verifyGrants.length - stillMissing.geographic}/${verifyGrants.length}`);
    console.log(`Grants with focus areas: ${verifyGrants.length - stillMissing.focusAreas}/${verifyGrants.length}`);
    console.log(`Grants with eligibility: ${verifyGrants.length - stillMissing.eligibility}/${verifyGrants.length}`);
    console.log(`Grants with deadlines: ${verifyGrants.length - stillMissing.deadline}/${verifyGrants.length}`);
    console.log(`Grants with duration: ${verifyGrants.length - stillMissing.duration}/${verifyGrants.length}`);
    console.log(`Grants with program type: ${verifyGrants.length - stillMissing.programType}/${verifyGrants.length}`);
