  ...
  for (const grant of grants) {
    ...
    try {
      ...
      if (grant.deadline.match(/\d{4}-\d{2}-\d{2}/)) {
        ...
        deadlineDate = new Date(`${parts[2]}-${parts[0].padStart(2, '0')}-${parts[1].padStart(2, '0')}`);
      } else {
...
async function amountValidatorAgent(grants) {
  ...
  for (const grant of grants) {
    ...
    if (grant.grant_size_min && grant.grant_size_max) {
      ...
      if (max > 10000000) { // More than 10 million
...
async function geographicValidatorAgent(grants) {
  ...
  const validRegions = ['Global', 'Europe', 'Eastern Europe', 'Ukraine', 'Asia', 'Africa', 'Americas'];
  const validCountries = ['Ukraine', 'Poland', 'Romania', 'Moldova', 'Georgia', 'Armenia', 'Belarus'];
  ...
  for (const grant of grants) {
    if (!grant.geographic_focus) {
      ...
      const isValid = validRegions.includes(grant.geographic_focus) || 
    ...
    try {
      ...
      const pageContent = await page.evaluate(() => {
        ...
        return {
          ...
          hasClosed: text.includes('closed') || text.includes('deadline passed') || text.includes('no longer accepting'),
          hasOpen: text.includes('apply now') || text.includes('submit application') || text.includes('open for applications'),
  ...
  try {
    ...
    results.forEach(agentResult => {
      ...
      if (agentResult.agent === 'Deadline Checker' && findings.expired.length > 0) {
        report.summary.criticalIssues.push(`${findings.expired.length} grants have expired deadlines`);
      }
