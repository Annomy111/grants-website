  ...
  try {
    ...
    for (const grant of grants) {
      ...
      if (!grant.geographic_focus) {
        ...
        if (grant.grant_name.toLowerCase().includes('global') || 
            grant.grant_name.toLowerCase().includes('international')) {
          updates.geographic_focus = 'Global';
        } else if (grant.grant_name.toLowerCase().includes('eastern europe') ||
      ...
      if (!grant.focus_areas_en || grant.focus_areas_en.length < 10) {
        ...
        for (const [keyword, areas] of Object.entries(focusAreaMapping)) {
          if (grantNameLower.includes(keyword) || 
      ...
      if (!grant.logo_url && grant.funding_organization) {
        ...
        if (logoMapping[orgSlug]) {
          ...
          // Try partial matches
          const partialMatch = Object.keys(logoMapping).find(key => 
            orgSlug.includes(key) || key.includes(orgSlug)
          );
    ...
    const { data: verifyGrants } = await supabase
      .from('grants')
      .select('id, grant_size_min, geographic_focus, focus_areas_en, eligibility_criteria_en, logo_url')
      .order('id');
    ...
    verifyGrants.forEach(grant => {
      ...
      if (!grant.eligibility_criteria_en || grant.eligibility_criteria_en.length < 20) stillMissing.eligibility++;
    ...
    console.log(`Grants with geographic focus: ${verifyGrants.length - stillMissing.geographic}/${verifyGrants.length}`);
    ...
    console.log(`Grants with logos: ${verifyGrants.length - stillMissing.logos}/${verifyGrants.length}`);
