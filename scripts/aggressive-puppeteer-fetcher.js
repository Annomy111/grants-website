...
const strategies = {
  ...
  ndi: ['https://www.ndi.org', 'https://www.ndi.org/about'],
  internews: ['https://internews.org', 'https://internews.org/about-us/'],
  luminate: ['https://www.luminategroup.com', 'https://luminategroup.com'],
  'norwegian-helsinki-committee': ['https://www.nhc.no', 'https://www.nhc.no/en/'],
  pivot: ['https://www.pivotsupport.org', 'https://www.pivot.org.au'],
  iri: ['https://www.iri.org', 'https://www.iri.org/who-we-are/'],
  'international-renaissance-foundation-irf': ['https://www.irf.ua', 'https://www.irf.ua/en/'],
};
  ...
  for (const [slug, urls] of Object.entries(strategies)) {
    ...
    for (const url of urls) {
      ...
      try {
        ...
        if (logoData.length > 0) {
          ...
          console.log(
            `    Found: ${bestLogo.width}x${bestLogo.height} - ${bestLogo.src.substring(0, 80)}...`
          );

          const ext = bestLogo.src.includes('.svg')
            ? '.svg'
            : bestLogo.src.includes('.png')
              ? '.png'
              : '.jpg';
    ...
    if (!logoFound) {
      ...
      if (backupUrl) {
        ...
        try {
          const ext = backupUrl.includes('.svg')
            ? '.svg'
            : backupUrl.includes('.png')
              ? '.png'
              : '.jpg';
          const tempPath = path.join(tempDir, `${slug}${ext}`);
