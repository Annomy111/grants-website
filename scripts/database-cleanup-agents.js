...
async function duplicateDetectionAgent() {
  ...
  for (const grant of grants) {
    ...
    const normalizedName = grant.name.toLowerCase().trim();
  ...
  const orgGroups = {};
  for (const grant of grants) {
    if (!orgGroups[grant.organization]) {
      orgGroups[grant.organization] = [];
    }
    orgGroups[grant.organization].push(grant);
  }
    ...
    if (orgGrants.length > 1) {
      for (let i = 0; i < orgGrants.length - 1; i++) {
        for (let j = i + 1; j < orgGrants.length; j++) {
          ...
          if (grant1.deadline === grant2.deadline && grant1.name !== grant2.name) {
            duplicates.push({
...
async function dataValidationAgent() {
  ...
  for (const grant of grants) {
    ...
    const requiredFields = ['name', 'organization', 'description_en', 'eligibility_en'];
    ...
    if (!grant.eligibility_en || grant.eligibility_en.length < 30) {
      stats.noEligibility++;
    ...
    if (grant.grant_size_min && grant.grant_size_max) {
      ...
      if (grant.grant_size_max > 10000000) {
        grantIssues.push('Unusually high max amount (> €10M)');
      }
    }
    
    // Check URLs
    const urlFields = ['website', 'application_url', 'logo_url'];
...
async function translationConsistencyAgent() {
  ...
  for (const grant of grants) {
    ...
    const translationFields = [
      ...
      { en: 'eligibility_en', uk: 'eligibility_uk' },
...
async function organizationLogoAgent() {
  ...
  for (const grant of grants) {
    const grantIssues = [];
    stats.organizations.add(grant.organization);
    ...
    if (!grant.logo_url || grant.logo_url === '') {
      ...
      if (logoMapping[grant.organization]) {
        grantIssues.push(`Logo available in mapping: ${logoMapping[grant.organization]}`);
      }
    ...
    const normalizedOrg = grant.organization.toLowerCase().replace(/[^a-z0-9]/g, '');
    if (!orgVariations.has(normalizedOrg)) {
      orgVariations.set(normalizedOrg, []);
    }
    orgVariations.get(normalizedOrg).push(grant.organization);
...
async function performanceOptimizationAgent() {
  ...
  if (grants.length > 100) {
    recommendations.push({
      type: 'indexing',
      priority: 'high',
      action: 'Ensure proper indexes on deadline, organization, and type fields',
